...
function floauth_init() {
	if ( isset( $_GET['floauth'] ) ) {
		...
		// Set floauth action.
		$floauth_action = sanitize_text_field( wp_unslash( $_GET['floauth'] ) );
		...
		if ( isset( $_GET['hash'] ) ) {
			...
			$role      = ( isset( $_GET['role'] ) && 'admin' === $_GET['role'] ? $admin_role : $member_role );
			...
			$redirect_url  = ( 'pages' === $floauth_action ? esc_url_raw( site_url( '/' ) . $redirect_path ) : esc_url_raw( admin_url() ) );
			...
			if ( $secret_key && $username && md5( $secret_key . $username ) === $hash ) {
				...
				if ( 'id' === $parameter_for_matching_user && $person_id ) {
